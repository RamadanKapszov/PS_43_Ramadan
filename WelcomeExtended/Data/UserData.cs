...
namespace WelcomeExtended.Data
{
    public class UserData
    {
        ...
        {
            return _users.FirstOrDefault(x => x.Name == name & x.Password == password);
        }
        public void SetActive(string name, DateTime date)
        {
            _users.FirstOrDefault(x=> x.Name== name).Expires = date;
        }
        public void AssignUserRole(string name, UserRolesEnum role)
        {
            _users.FirstOrDefault(x => x.Name == name).Role = role;
        }
    }
}
